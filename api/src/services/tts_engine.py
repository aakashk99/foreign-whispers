...
def _synthesize_raw(tts_engine, text: str, wav_path: str, speaker_wav: str | None = None) -> bytes | None:
    ...
    *speaker_wav* is an optional path to a reference WAV for voice cloning.
    When provided it is forwarded to ``tts_to_file`` as a kwarg so that
    ``ChatterboxClient`` uses the ``/v1/audio/speech/upload`` endpoint.
    """
...
def text_file_to_speech(source_path, output_path, tts_engine=None, *, alignment=None, speaker_wav_map: dict | None = None):
    ...
    *alignment* overrides the module-level ``_ALIGNMENT_ENABLED`` flag.
    ...
    for i, seg in enumerate(segments):
        ...
        seg_metas.append({
            ...
            "speaker_wav": (speaker_wav_map or {}).get(seg.get("speaker", ""), None),
        })
